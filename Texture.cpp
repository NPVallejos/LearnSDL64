...
namespace Nick
{
	...
	void Texture::render(SDL_Renderer* renderer, int x, int y, int resizeFactorX, int resizeFactorY, SDL_Rect* clip)
	{
		SDL_Rect renderQuad = { x * resizeFactorX, y * resizeFactorY, width * resizeFactorX, height * resizeFactorY };

		if (clip != NULL)
		{
			renderQuad.w = clip->w * resizeFactorX;
			renderQuad.h = clip->h * resizeFactorY;
		}
